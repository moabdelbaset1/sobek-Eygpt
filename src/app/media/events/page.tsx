...
import { Calendar, MapPin, Clock, ArrowRight, Search, X, Play, Sparkles, Filter } from 'lucide-react';
...
export default function EventsPage() {
  ...
  const [filteredEvents, setFilteredEvents] = useState<MediaPost[]>([]);
  ...
  useEffect(() => {
    if (searchQuery.trim() === '') {
      ...
      const filtered = events.filter(event =>
        ...
        (event.title_ar && event.title_ar.includes(query)) ||
        event.content.toLowerCase().includes(query) ||
        (event.content_ar && event.content_ar.includes(query))
      );
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Hero Section */}
      <div className="relative h-[50vh] min-h-[400px] bg-rose-900 overflow-hidden">
        <div className="absolute inset-0">
          <Image
            ...
            priority
          />
          <div className="absolute inset-0 bg-gradient-to-b from-rose-900/80 via-rose-900/60 to-gray-50"></div>
        </div>
        ...
        <div className="relative z-10 container mx-auto px-4 h-full flex flex-col justify-center items-center text-center">
          ...
          >
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-rose-800/50 backdrop-blur-sm rounded-full text-rose-200 mb-6 border border-rose-700/50">
              <Calendar className="w-4 h-4" />
              <span className="text-sm font-medium uppercase tracking-wider">
                {lang === 'ar' ? 'فعاليات ومؤتمرات' : 'Events & Conferences'}
              </span>
            </div>
            <h1 className="text-4xl md:text-6xl font-bold text-white mb-6 leading-tight">
              {lang === 'ar' ? 'أحدث الفعاليات' : 'Latest Events'}
            </h1>
            <p className="text-xl text-rose-100 max-w-2xl mx-auto leading-relaxed">
              {lang === 'ar' 
                ? 'تابع أحدث أخبارنا ومشاركاتنا في المؤتمرات والفعاليات الطبية.'
                : 'Stay updated with our latest news, conference participations, and medical events.'}
            </p>
          </motion.div>
          ...
          >
            <div className="relative group">
              <div className="absolute inset-0 bg-rose-400 rounded-full blur opacity-25 group-hover:opacity-40 transition-opacity duration-300"></div>
              <div className="relative bg-white rounded-full shadow-xl flex items-center p-2">
                ...
                />
                <button className="bg-rose-600 text-white px-6 py-2 rounded-full font-medium hover:bg-rose-700 transition-colors shadow-md">
                  {lang === 'ar' ? 'بحث' : 'Search'}
                </button>
              </div>
            </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20 bg-white rounded-3xl shadow-xl border border-gray-100">
            ...
            <p className="text-gray-500 max-w-md mx-auto">
              {lang === 'ar' 
                ? 'لم نتمكن من العثور على أي فعاليات تطابق بحثك.'
                : 'We couldn\'t find any events matching your search.'}
            </p>
            <button 
              onClick={() => setSearchQuery('')}
              className="mt-6 text-rose-600 font-semibold hover:text-rose-800 transition-colors"
            >
                  ...
                  >
                    <div className="bg-white rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-500 border border-gray-100 h-full flex flex-col hover:-translate-y-2">
                      ...
                      >
                        {event.media_url ? (
                          ...
                          <div className="w-full h-full flex items-center justify-center bg-rose-50">
                            <Calendar className="w-12 h-12 text-rose-200" />
                          </div>
                        )}
                        
                        {/* Date Badge */}
                        <div className="absolute top-4 left-4 bg-white/90 backdrop-blur-sm rounded-xl p-2 text-center shadow-lg min-w-[60px]">
                          <span className="block text-xs font-bold text-rose-600 uppercase">{date.month}</span>
                      ...
                      <div className="p-6 flex flex-col flex-grow">
                        <div className="flex items-center gap-2 text-xs font-medium text-rose-600 mb-3">
                          <Sparkles className="w-3 h-3" />
                          <span className="uppercase tracking-wider">
                            {event.type === 'event' ? (lang === 'ar' ? 'فعالية' : 'Event') : (lang === 'ar' ? 'خبر' : 'News')}
                          </span>
                        </div>
                        ...
                        <div className="pt-6 border-t border-gray-100 flex items-center justify-between mt-auto">
                          ...
                          <button 
                            onClick={() => setExpandedMediaId(event.id)}
                            className="text-sm font-semibold text-rose-600 flex items-center gap-1 group-hover:gap-2 transition-all"
                          >
                ...
                item.id === expandedMediaId ? (
                  <div key={item.id} className="flex flex-col md:flex-row h-[80vh]">
                    ...
                    <div className="w-full md:w-1/3 bg-white p-8 overflow-y-auto">
                      <div className="flex items-center gap-2 text-sm font-bold text-rose-600 mb-4 uppercase tracking-wider">
                        <Calendar className="w-4 h-4" />
