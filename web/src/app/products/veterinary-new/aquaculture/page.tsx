...
export default function AquaculturePage() {
  ...
  const loadProducts = async () => {
    try {
      setLoading(true);
      const data = await veterinaryProductsAPI.getByCategory('aquaculture');
      setProducts(data);
    } catch (error) {
      ...
      <div className="bg-gradient-to-r from-cyan-600 to-cyan-700 text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold mb-2">🐟 Aquaculture</h1>
              <p className="text-cyan-100">Fish and aquatic animal health</p>
            </div>
            <div className="bg-white/20 backdrop-blur-sm rounded-lg px-4 py-2">
              <p className="text-sm text-cyan-100">Total Products</p>
                ...
                <div className="relative h-48 bg-gradient-to-br from-cyan-50 to-cyan-100">
                  {product.image_url ? (
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <Package className="w-16 h-16 text-cyan-300" />
                    </div>
                  )}
                </div>
