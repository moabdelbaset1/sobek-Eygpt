...
export default function PoultryHealthPage() {
  ...
  const loadProducts = async () => {
    try {
      setLoading(true);
      const data = await veterinaryProductsAPI.getByCategory('poultry-health');
      setProducts(data);
    } catch (error) {
      ...
      <div className="bg-gradient-to-r from-yellow-600 to-yellow-700 text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold mb-2">🐔 Poultry Health</h1>
              <p className="text-yellow-100">Treatments for chickens, ducks, and birds</p>
            </div>
            <div className="bg-white/20 backdrop-blur-sm rounded-lg px-4 py-2">
              <p className="text-sm text-yellow-100">Total Products</p>
