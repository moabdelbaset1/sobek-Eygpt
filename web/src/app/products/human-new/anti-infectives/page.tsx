...
import { Shield, ArrowLeft, Package, ShoppingCart, Info } from 'lucide-react';
...
export default function AntiInfectivesPage() {
  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        const data = await getProductsByCategory('anti-infectives');
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              ...
              <div className="flex items-center">
                ...
                <div>
                  <h1 className="text-3xl font-bold text-gray-900">Anti-Infective Medications</h1>
                  <p className="text-gray-600 mt-2">Antibiotics, Antivirals & Antifungals</p>
                </div>
              </div>
            </div>
            <div className="flex items-center px-4 py-2 bg-blue-50 rounded-lg">
              <Package className="w-5 h-5 text-blue-600 mr-2" />
              <span className="text-blue-700 font-medium">{products.length} {products.length === 1 ? 'Product' : 'Products'}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-gray-500">Anti-infective medications will be added soon</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {products.map((product, index) => (
              ...
              >
                {/* Product Image */}
                <div className="relative h-48 bg-gradient-to-br from-blue-50 to-blue-100 rounded-t-xl overflow-hidden">
                  {product.image_url ? (
                    <img 
                  ...
                  <div className="absolute top-4 right-4 bg-white px-2 py-1 rounded-full text-xs font-medium text-blue-600">
                    {product.dosage_form}
                  </div>
                </div>
