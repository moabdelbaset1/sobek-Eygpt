...
import { Shield, ArrowLeft, Package, ShoppingCart, Info, Activity } from 'lucide-react';
...
export default function EndocrinologyPage() {
  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        const data = await getProductsByCategory('endocrinology-diabetes');
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              ...
              <div className="flex items-center">
                ...
                <div>
                  <h1 className="text-3xl font-bold text-gray-900">Endocrinology & Diabetes Medications</h1>
                  <p className="text-gray-600 mt-2">Diabetes Management & Hormonal Therapy</p>
                </div>
              </div>
            </div>
            <div className="flex items-center px-4 py-2 bg-green-50 rounded-lg">
              <Package className="w-5 h-5 text-green-600 mr-2" />
              <span className="text-green-700 font-medium">{products.length} {products.length === 1 ? 'Product' : 'Products'}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-gray-500">Endocrinology & diabetes medications will be added soon</p>
          </div>
        ) : (
                ...
                <div className="relative h-48 bg-gradient-to-br from-green-50 to-green-100 rounded-t-xl overflow-hidden">
                  {product.image_url ? (
                    ...
                    <div className="flex items-center justify-center h-full">
                      <Activity className="w-16 h-16 text-green-300" />
                    </div>
                  )}
                  <div className="absolute top-4 right-4 bg-white px-2 py-1 rounded-full text-xs font-medium text-green-600">
                    {product.dosage_form}
                  </div>
                </div>
